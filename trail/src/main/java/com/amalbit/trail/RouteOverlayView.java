    ...
    for (OverlayPolyline overlayPolyline : overlayPolylines) {
      if (overlayPolyline.getProjectionHelper().getCenterLatLng() != null) {
        overlayPolyline.getProjectionHelper().onCameraMove(
            googleMapProvider.getGoogleMapWeakReference().getProjection(),
            googleMapProvider.getGoogleMapWeakReference().getCameraPosition()
        );
      }
...
//  }
  ...
  private boolean isGoogleMapNotNull() {
    return googleMapProvider != null && googleMapProvider.getGoogleMapWeakReference() != null &&
