...
public class DirectionsJSONParser {
    ...
    Float viaPointsList = 0.0f;
    public static String milesString = null;
    public static String trackingmilesString = null;
    private String jOverviewPoly;
    public static String shortTime = "";
    boolean isVia = false;

    public DirectionsJSONParser(boolean isVia) {
        this.isVia = isVia;
    }
...
//					}
                    ...
                    if (isVia) {
                        String viaPoints = jDistance.getString("text").replace(" mi", "");
                        viaPointsList += Float.parseFloat(viaPoints);
                    }
...
//					routes.add(path);
            ...
            for (int j = 0; j < jLegs.length(); j++) {
                JSONObject jDistance = ((JSONObject) jLegs.get(j)).getJSONObject("distance");
                Indistance = Integer.parseInt(jDistance.getString("value"));
                distance.add(Indistance);
                //	int JsonDis=jDistance.getString("")
                if (j > 0 && jLegs.length() > 1) {
                    JSONObject viaPoints = ((JSONObject) jLegs.get(j)).getJSONObject("start_location");
...
//				}
            ...
            float miles = convertToMiles(Indistance);
            if (isVia) {
                miles = viaPointsList;
            }
            milesString = "Your estimated journey time is " + convertToHours(durationInt / 60) + " and journey distance is " + String.format("%.1f", miles)
                    + " miles.";
            trackingmilesString = shortTime + "\n" + String.format("%.1f", miles)
                    + " mi";
...
//					+ " miles.";
    ...
    private String convertToHours(int minutes) {
        if (minutes >= 60) {
            ...
            if (minutes % 60 != 0) {
                ret += " & " + convertToHours(minutes % 60);

            }
            shortTime = shortHours(minutes);
            return ret;
        } else {
            if (minutes > 1) {
                shortTime = shortHours(minutes);
    ...
    public static String shortHours(int minutes) {
        if (minutes >= 60) {
            int hours = minutes / 60;
            String ret = "";
            if (hours > 1) {
